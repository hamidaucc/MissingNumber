...
def getMissingNumber(x):
    ...
    total=(length*n)//2
    #total=(n*(n+1))//2
    print(f"Formula[n*((n+1)/2] :{total}")
    s=sum(x)
    print(f"Sum of length: {s}")
    
    print(f"Difference is [total - sum]: {total -s}")
    return total -s
#a=[9,6,4,2,3,5,7,0,1]
#a=[0,1]
a=[3,0,1]
print(f"Missing number is \t: {getMissingNumber(a)}")
